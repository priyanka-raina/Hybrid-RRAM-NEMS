...
//	
//
//  Source file: /home/akashl/CGRAGenerator/hardware/generator_z/pe_new/pe/rtl/test_pe.svp
...
//
// reg_out (_GENESIS2_INHERITANCE_PRIORITY_) = 1
//
// use_add (_GENESIS2_INHERITANCE_PRIORITY_) = 2
//
...
//
// mult_mode (_GENESIS2_INHERITANCE_PRIORITY_) = 2
//
...
assign en_acc = op_code[9:9];
  ...
  assign op_b_ld  = (cfg_en && (cfg_a == 8'hF1)) | (clk_en & (cntr_en|en_acc));
  assign op_b_val = (cfg_en && (cfg_a == 8'hF1)) ?
                    cfg_d[DataWidth-1:0] :
                    ((cntr_en & comp_res_p)|(en_acc & op_d_p) ? {DataWidth{1'b0}}: comp_res);
...
test_opt_reg #(.DataWidth(DataWidth)) test_opt_reg_c
(
  .clk        (clk),
  .clk_en     (clk_en),
  .rst_n      (rst_n),
...
always_ff @(posedge clk or negedge rst_n) begin
  if(~rst_n) begin
    ...
    res   <= (cntr_en | en_acc) ? op_b : comp_res;
    res_p <= res_p_w;
  end
end
